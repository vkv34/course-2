...
fun MainEquipmentList() {
    ...
    val barcodeScannerLauncher = rememberLauncherForActivityResult(
        ...
        onResult = {
            try {
                navController.navigate(AppNavigation.User.EquipmentDetails.putId(it.contents))
            } catch (e: Exception) {
                scope.launch {
...
//            IconButton(onClick = {
//                navController.navigate(AppNavigation.Engeener.MyRequestsScreen.route)
//            }) {
//                Icon(
...
//            }
        ...
        EquipmentList(records = equipments, onEquipmentClick = {
            navController.navigate(AppNavigation.User.EquipmentDetails.putId(it.inventoryNumber))
